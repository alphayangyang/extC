...
**验收**：`let x: i32 = "abc"` 由 extC **自己**报错（现在漏给 gcc，靠 `#line` 兜着）。

### T3 · `ref` 升级成表达式 + 调用点显式（定案 10）

- `ref` 从「类型修饰」升级成「**表达式**」：`ref s.board` 产生一个引用值
- 自由函数的 `ref T` 实参**必须在调用点写 `ref`**
- **方法接收者自动取地址**（`.` 表示「在这个值上操作」）
- 类型层要为**逃逸检查留位**（现在不做检查，但类型表示要能挂 arena 深度）
...
| 定案 | 现状 |
|---|---|
| 8 · 默认零初始化（`var b: Board` 合法） | week-0 会报 `needs an initializer`，要改掉 |
| 9 · 方法写在 struct 体内 | 现在是自由函数 + `self` 首参数 |
| 13 · `type` 枚举（`type Status = \| ok \| warn`） | 没做 |
