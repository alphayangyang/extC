...
**`slice<T>` 已经存在了**（T4b 起就在 `stdlib/prelude.extc` 里，而且**字符串就是它**）。
这里要补的是**从数组切出切片**的语法 `a[i..j]`。
...
`b[2..5][3..8]` **不支持**（不连续，没法用一个指针+长度表示）✓ 保留旧文档的判断。

### 逃逸

切片**不拥有数据**，所以它绑原数组的生命周期。`fn bad() -> slice<i32> { let a: [15]i32; return a[..] }`
必须被拒绝 —— **等 week-4 的引用规则**（现在引用规则还没落地，这条检查也还没有）。
