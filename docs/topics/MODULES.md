...
`SPEC.md` §8.1 的"档 3"里记着）⇒ 所以 extC 的 IO 需要**两条腿**：
...
| 没有线程、明确不做开放注册 | `SPEC.md` §7 | ⇒ 初始化顺序问题**可以简化**（没有并发、没有析构、没有开放注册）✓ |
...
6. **同一套常设验收照跑**：**257** 测试 · ASan 8 · arena 5 · 攻击库基线一字不动 ✓
...
3. ~~没有 `as` 别名（短名 = 路径最后一段）~~ ✅ **已解**（2026-09-23，PLAN #53）：
   **两件事都做了** —— ① **全名是权利**：表达式位置现在支持**任意层**限定名
   （`std::sys::io::STDOUT` · `std::sys::io::write(…)` · `lib::sub::color.color.green`）✓
   ② **别名是方便**：`use std::sys::io as sysio` ⇒ 之后写 `sysio::STDOUT` ✓
   ⚠️ 一条不变式：**别名只换 `shortName`，`path` 永远是用户写的全名** ✗
   （`path` 是"全名对全名"匹配 `use` 表的依据 —— 深层限定名能解开全靠它 ✓，
   `tests/qname/run.sh` 里有一条结构判据守着这件事 ✓）
   常设验收 `tests/qname/`（6 正例 + 1 反例 + 1 结构判据）：其中 **`t14` 是这条限制真正的痛点** ——
   `use std::io` 与 `use std::sys::io` **在同一文件里共存**（加别名即可 ✓），
   而这两层恰好是"从 stdin 读 + `open` 一个文件"要用的组合 ✓
   ⚠️ 主人原话：「**为什么用户不能写 `std::sys::io::read` 这应该是被允许的**，
   但是别名是必要的，否则就太长了，用 `as` 即可」—— **全名是权利，别名是方便** ✓
   还没做：**per-module 的 `use` 可见性传递**（`use` 不重新导出 ✓）
