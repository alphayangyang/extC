...
>
> 现在的标准见 [`DESIGN.md`](../../docs/DESIGN.md)（一条原则）和 [`DECISIONS.md`](../../docs/DECISIONS.md)（已定案）。
...
`tests/run.sh` **255 通过**（2026-09-24 实测；原文写 60、后来写 251、再后来写 257 ✗ ——
257 → 255 是 `2eb015d` 把 **2 条**搬进了 `tests/arena-promoted/`（**换套件、覆盖没丢**，
两条现在都还在跑 ✓）；另有 `tests/arena-promoted/` **22/22** 不计入这个数 ✓
—— **这正是"实测"两个字也要带日期和命令**的理由：它会变，而且变的原因不止一种 ✓）。
