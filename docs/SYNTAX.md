...
（`close` 不在这张表里：**块拥有文件** ⇒ 块退出自动关 ✓；
逃生舱 `close(f)!` 与写型的 `commit()?` 见 `IO.md` §5 / **定案 78**）
（⚠️ 2026-09-24 修正：原来写"帧拥有"+"`close(f)!` 是 IO-2 的事"——两处都改了 ✗）
...
`std::fs` 模块本身 · **块拥有文件**（定案 78）· `readAll` · `f.reader()` ·
