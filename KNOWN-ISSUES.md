### ✅ 已解决（2026-09-23，层 1「数据流第一步」）

**`examples/field-strong-update.extc` 的误拒已经修好** —— `tests/run.sh` 现在 **255 通过 / 0 失败**，
而且哨兵 `tests/arena-soundness/H1_strongupdate_missed.extc` **仍然被拒** ✓

**修法（只加了"事实完不完整"这一位，31 行）**：
给 `Sym` 加 `fieldsComplete`；**结构体字面量的表是完整的**（写出来的都在表里；省略的要么是
可空引用（零值 null ⇒ 深度 0），要么是"没有零值"的引用 ⇒ 检查器**当场报错**）。
于是"那一格被新值替代 ⇒ 从根的重算里去掉它"（= `ARENA-FORMAL` §7.4 的**强更新**）只在
**表完整**时启用；**表不完整一律不许下降** ✓

**为什么之前三次都失败**：前两次是漏了"两条码路"（普通赋值 / **引用型目标**）；
第三次漏的是**表可以不完整** —— `var h3 = h` 之后 `h3` 的表里没有 `q` 那一格，
于是"去掉被写的 `p`"之后 `q` 的深度**凭空消失** ⇒ `return h3` 被放行 ⇒
真的 `stack-use-after-scope` ✗（哨兵当场抓到，`tests/run.sh` 那时是假的 255/0）✓

---

### （存档）已知误拒：2026-09-23 合并 arena 健全性修复时记入
