...
# 附录 F：**gdb 才是这一轮的转折点**（2026-09-23 收官）

> 主人一句话点醒：「**测试的时候多用 gdb**」✗ —— 前几轮我全靠 `fprintf` 探针 + 猜，
> 一轮一轮试；这一轮改用 gdb，**四个真根因一次看穿** ✓
> 净成果：两处真 bug 修进仓库（`584a5e4`），主套件 **255/0** ✓、转正库 **18/22**（不变）
> ⚠️ 还试出"第三处"能让转正库到 **20/22**，但它会弄红两条绿用例 ⇒ **未采用**，形状记在 F.3 ✓

## F.1 gdb 怎么用（这套法子下次直接用）

`build/extc` 本来就带 `-g`（Makefile 的 `CFLAGS` 里有）⇒ **不用改构建**，直接：

```sh
gdb -q -batch -x <脚本> ./build/extc
```
...
```gdb
# ① 断在**某个具体分支**上（源文件名:行号）
break check_escape.c:563
# ② 带条件（按符号名/行号过滤掉 prelude 的噪声 —— 关键！否则几百条）
break check_escape.c:543 if sy->name[0] == 'p' || sy->name[0] == 'o'
# ③ 自动跑、自动打、不交互
commands
  silent
  printf "sym=%s depth=%d refDepth=%d org=%d at=%d\n", sy->name, sy->depth, sy->refDepth, sy->origin ? (int)sy->origin->kind : -1, at
  continue
end
run <源文件> -o /dev/null
```
...
1. **一定要加条件**：不加的话 prelude 的几百次调用会把输出淹掉 ✗
2. **`%d` 配 `(int)` 强转**：枚举/指针直接喂 `%d` 会让 gdb 报
   "Error in sourced command file" 然后整段脚本**静默失效**（我浪费了两轮 ✗）；
3. **断点行号 = 变量已经赋值的那一行**：断在 `int at0 = storeLayer(...)` 那一行时
   `at0` 还是未初始化 ⇒ 要断在**下一行** ✓
4. **能问函数就问**：`printf "%d", exprRefDepth(c, val)` 比"再插一个 print 重新编译"
   快一个数量级 ✓（注意：调试**编译器自己**时，被调函数是编译器里的函数，
   gdb 直接调它没有副作用 ✓）

## F.2 这一轮 gdb 查出来的两处真 bug（已修，`584a5e4`）

| # | 现场 | 根因 |
|---|---|---|
| **1** | `p = x` 之后 `p->origin` 还是 `EX_NULL`（`org=24`）| `noteOrigin` 没有守卫，**顺序**检查里 `p = null` 把 `p = x` 记下的来路**盖掉**了 ⇒ 追不到站点 ⇒ 悬垂 ✗（修法：字面量不许盖掉"可追的来路"✓）|
| **2** | 站点 `refDepth=0` 而按层号算应为 1 —— **自己跟自己矛盾** | 收尾那句 `site->refDepth = …` 包在 `if (arenaLevel != want)` 里 ⇒ "预负已是 HOME、解算又定成 HOME"的站点层号不变 ⇒ **那句话一次都不执行** ⇒ `refDepth` 停在中途被改坏的值上 ✗（修法：定案后无条件同步 ✓）|

> **教训**：`refDepth` 与 `arenaLevel` 是**同一个数的两种说法** —— 凡是"只在某个条件分支里
> 更新其中一个"的地方，都要问一句"另一个还同步吗"✓

## F.3 第三处：**能让转正库到 20/22，但代价不划算**（未采用，下次从这里接）

**是什么**：`exprRefDepth` 顶部那个缓存早退

```c
if (!c->substParams && !mentionsParam(e->type) && e->refDepth) return e->refDepth;
```

对**引用型绑定**（`EX_IDENT` 且类型是 `TY_REF`）跳过这个缓存 ⇒ 改问绑定 ✓

**为什么能让 C2 转绿**（gdb：`exprRefDepth(p)` 第一次答 **0**、第二次答 **2**）：

```
var p: ?ref i32 = null      // 表达式节点上缓存 = 1（槽位那一层）
if c == 1 { p = x }         // p 换指向 ⇒ 绑定 refDepth 更新成 0 ✓，节点缓存不动 ✗
out = p                     // 读到节点上的旧缓存 ⇒ "1" 一路传到 out ⇒ 误拒 ✗
```

**为什么没采用**：加上它之后 —— 转正库 **18→20** ✓，但主套件 **255/0 → 253/2** ✗
（`examples/store-promotion` 的 `head = mid`、`tests/asan/promote-through-mid` 编译期报错）✗
实测隔离过：**就是这一处**导致的（关掉它 = 255/0；开着它 = 253/2）✓

**根因（这一轮没查完）**：那两个例子的来路是一条**链**
（`head → mid → cell → new`），而"跳过缓存"之后走到的计算路径给出的数**与缓存不一致** ——
gdb 看到同一个节点两次调用分别答 0 和 2 ⇒ 说明**有两条路径在算同一个数、结果不同** ✗
⇒ 下次要做的**不是**"跳过缓存"，而是**让两条路径算出同一个数**（把"站点深度"这件事
收敛到**一个函数**里，缓存只存它的结果）✓ —— 这正是定案 68「唯一权威」那条纪律在
*深度*这一侧的翻版 ✓
...
| 判据 | 现在 | 目标 |
|---|---|---|
| `tests/run.sh` | **255 / 0** ✓ | 255/0 ✓ |
| 反例库洞还在 | **0** ✓ | 0 ✓ |
| 转正库 | **18 / 22** | **22/22** ✗ |
| asan / arena | 8 / 5 ✓ | 8 / 5 ✓ |
| 攻击库 | 与基线一字不差 ✓ | ✓ |
| `r1` 峰值 | **50 808 KB** ✓ | ≈50.8MB ✓ |

**下一步（一句话）**：把"某个值现在有多深"收敛成**一个函数**
（`exprRefDepth` / `valDepthForStore` / `valDepthStructural` / `solvedValDepth` 现在有**四份**
各算各的 —— 附录 D/F 那些"同一节点两个答案"全是这么来的 ✗）⇒
先写一个**对拍探针**（同一个值问四个函数，不一致就报），把不一致的地方**逐个消掉** ✓
—— 这比继续打地鼠便宜得多，而且它是**判据**，不是猜测 ✓
