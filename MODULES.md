...
3. 没有 `as` 别名（短名 = 路径最后一段）；没有 per-module 的 `use` 可见性传递（`use` 不重新导出 ✓）
   ⚠️ **这一条 2026-09-23 复核时补上了它的"另一半"**：别名不只是"方便"，
   **全名写不出来才是真缺口** —— **表达式位置**的三段及以上限定名编不过
   （`std::sys::io::STDOUT` ⇒ `expected an expression, found \`::\``），
   而**类型位置**本来就支持任意层 ✓ ⇒ 于是同一文件里 `use std::io` 与 `use std::sys::io`
   **共存不了**（短名都是 `io`，报 "two modules with the same last name cannot be told apart"）✗
   ⇒ 已记成 **`PLAN.md` §0.4 #53**（带五条失败记录与下一轮做法）✓
   ⚠️ 这条限制的**主人原话**是「为什么用户不能写 `std::sys::io::read` 这应该是被允许的，
