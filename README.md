...
⚠️ 三个用例套件由 `tools/parrun.py` **并行**跑（每个用例都要起 extc + gcc，串行时几十核只用 1 个 ✗）。
输出**按提交顺序回放** ⇒ 与串行逐字节一致、跑两遍也逐字节一致 ✓
并发数：`PAR_JOBS=<n>`（默认见 `tools/parrun.py`；不是 cpu_count —— 编译器和 gcc 各自还会派子进程）。
实测（2026-09-24，24 核）：`tests/run.sh` **35.7 秒**（原 76.8）· `check.sh quick` **76.8 秒**（原 94.6）✓
