...
`./check.sh quick` **11 节全绿**：**257** 测试 · arena 5 · 警告零误报 · 泛型 3 · extern 4 ·
模块 12 · IO 10 · ASan 8 · 层号哨兵 98 语料零漂移 · 攻击库与基线一致 ✓
（完整模式 **17 节** —— 多出的 6 节是基准 ✓ `tools/golden.sh check` 是**另一个脚本**：
...
⭐ **另外有一条挡路的缺口**：[`PLAN.md`](PLAN.md) §0.4 **#53** —— **表达式位置写不出
三段及以上的全限定名**（`std::sys::io::STDOUT` ⇒ `expected an expression, found \`::\``），
而且 `as` 别名也没有 ⇒ 同一个文件里 `use std::io` 与 `use std::sys::io` **共存不了**
（短名都是 `io`）✗ 两段的 `mod::name` 和**类型位置**的任意层名字都是好的 ✓
（`DEVLOG.md` 顶部那条记着已失败的五次尝试与下一轮的做法 ✓）
...
正例跑通 + 反例必须被编译期挡掉（当前 **257 通过 / 0 失败**）✓
