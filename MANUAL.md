...
⚠️ **一条故意不查的**（已记成欠账）：**把引用传进函数、函数把它存起来**
（`fn stash(s: slice<i32>) { g.s = s }`）。那需要引用的「传染性」分析，
比深度模型贵得多，见 [`REFS.md`](REFS.md) §6 未决问题 5 ——
今天也走不通（没有全局变量、引用字段只能在构造时设置）。

> 深度模型**不是为今天设计的，是为内存模型设计的**：
> 等全局变量（深度 0）和 arena 到位，`g.s = a[..]` 这类就会**自动**被同一条规则挡住。
