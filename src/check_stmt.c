        ...
        case ST_ASSIGN: {
            ...
            if (tt_->kind == TY_REF) {
                ...
                if (vt0->kind == TY_REF) {
                    ...
                    /* ⭐⭐ 层 2：**"这个引用要活到第几层"由容器的寿命封顶** ✗✗
                     * `storeLayer(h.q)` 走 `placeDepth` ⇒ 答**投影那条路**的深度（2），
                     * 而 `h` 自己就住在**第 1 层**（本地绑定活不过它那一层作用域）✗
                     * ⇒ 站点按 2 定案 ⇒ 块一退就 release ⇒ 而 `h.q` 里那个指针
                     *   已经跟着 `out = h` 逃到帧外 ⇒ **悬垂**（C1，ASan 实锤）✗✗
                     * 判据：**本帧里没有任何东西能活过当前作用域** ⇒
                     * `c->scopes.len` 就是"这一次存储能要求的最浅层" ✓
                     * ⚠️ 只封顶**记账用的那个数**；下面 `checkEscape` 的 `at0` 一个字不动 ✓ */
